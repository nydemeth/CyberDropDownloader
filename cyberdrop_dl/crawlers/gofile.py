...
class GoFileCrawler(Crawler):
    ...
    async def __async_post_init__(self) -> None:
        with self.catch_errors(self.api.ENTRYPOINT), self.disable_on_error("Unable create temp account"):
            if key := self.config.auth.gofile.api_key:
                ...
                self.api.key = await self.api.create_temp_account()
    ...
    async def folder(self, scrape_item: ScrapeItem, content_id: str, single_file_id: str | None = None) -> None:
        node, pages = await aio.peek_first(self.api.content(content_id, scrape_item.password))
        if _has_single_not_nested_file(scrape_item, node):
            ...
            title = self.create_title(name := node["name"], content_id, force_album_id=name == "root")
        ...
        async for node in pages:
            ...
            if single_file_id:
                ...
                self._iter_children(scrape_item, [file])
                return

            self._iter_children(scrape_item, children.values())

    def _iter_children(self, scrape_item: ScrapeItem, children: Iterable[Node]) -> None:
