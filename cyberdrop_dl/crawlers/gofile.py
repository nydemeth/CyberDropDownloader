...
class GoFileCrawler(Crawler):
    ...
    async def folder(self, scrape_item: ScrapeItem, content_id: str, single_file_id: str | None = None) -> None:
        ...
        async for node in self.api_pager(content_id, scrape_item.password):
            if is_first_page:
                if _has_single_not_nested_file(scrape_item, node):
                    # Consider this file a loose file (autogenerated folder name)
                    title = ""
                    part_of_album = False
                else:
                    name = node["name"]
                    title = self.create_title(name, content_id, force_album_id=name == "root")
                    part_of_album = True
            ...
            children = {node["id"]: node} if node["type"] == "file" else node["children"]
    ...
    async def api_pager(self, content_id: str, password: str | None = None) -> AsyncGenerator[Folder | File]:
...
def _has_single_not_nested_file(scrape_item: ScrapeItem, node: Folder | File) -> bool:
    return (
        node["type"] == "file"
        or node["childrenCount"] == 1
        or (node["name"] == node["code"] and scrape_item.type != ScrapeItemType.ALBUM)
    )
