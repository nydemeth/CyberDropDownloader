DOMAIN = "toonily"
TEST_CASES = [
    {
        "url": "https://toonily.com/serie/black-crow/",
        "results": [
            {
                "url": "https://data.tnlycdn.com/chapters/manga_66d8f7bfa8ed9/03ae6f538b989573c121b28f7af4f38a/01.jpg",
                "filename": "01.jpg",
                "referer": "https://toonily.com/serie/black-crow-b43fe3e6/chapter-1",
                "album_id": None,
                "uploaded_at": 1725498328,
                "download_folder": "re:Black Crow (Toonily)/Chapter 1",
            }
        ],
        "count": 1634,
    },
    {
        "url": "https://toonily.com/serie/black-crow-b43fe3e6/chapter-1/",
        "results": [
            {
                "url": "https://data.tnlycdn.com/chapters/manga_66d8f7bfa8ed9/03ae6f538b989573c121b28f7af4f38a/01.jpg",
                "filename": "01.jpg",
                "referer": "https://toonily.com/serie/black-crow-b43fe3e6/chapter-1",
                "album_id": None,
                "uploaded_at": 1725498328,
                "download_folder": "re:Black Crow (Toonily)/Chapter 1",
            }
        ],
        "count": 50,
    },
    {
        "url": "https://toonily.com/serie/glorious-homecoming-440a49b5/chapter-6/",
        "results": [
            {
                "url": "ANY",
                "referer": "https://toonily.com/serie/glorious-homecoming-440a49b5/chapter-6",
                "album_id": None,
                "download_folder": "re:Glorious Homecoming (Toonily)/Chapter 6",
            }
        ],
        "count": 13,
    },
]
