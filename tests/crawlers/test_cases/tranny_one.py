DOMAIN = "tranny.one"
TEST_CASES = [
    {
        "url": "https://es.tranny.one/view/1195231",
        "results": [
            {
                "url": "re:https://stream.tranny.one/.+/3176393.mp4",
                "filename": "Travesti pelirroja voluptuosa muy intensa y sudorosa. [1195231].mp4",
                "referer": "https://es.tranny.one/view/1195231",
                "uploaded_at": None,
            }
        ],
    },
    {
        "url": "https://www.tranny.one/search/ruby+wren/",
        "results": [],
        "count": range(5, 10),
    },
    {
        "url": "https://www.tranny.one/pics/album/2967/",
        "results": [
            {
                "url": "re:https://pics.tranny.one/work/orig/2904/339248",
                "download_folder": "re:Natalie mars cuckolds the world \\[album\\] \\(Tranny\\.One\\)",
                "referer": "https://www.tranny.one/pics/album/2967",
                "album_id": "2967",
            }
        ],
        "count": 17,
    },
    {
        "url": "https://pics.tranny.one/work/orig/2915/385865/1.jpg",
        "results": [
            {
                "url": "https://pics.tranny.one/work/orig/2915/385865/1.jpg",
                "filename": "1.jpg",
                "download_folder": "re:Tranny\\.One",
                "referer": "https://pics.tranny.one/work/orig/2915/385865/1.jpg",
            }
        ],
    },
]
